...
using Microsoft.EntityFrameworkCore;
    ...
    public class GenericRepository<TEntity> : IGeneric<TEntity> where TEntity : class
    {
        ...
        public async Task<List<TEntity>> GetPagedIndexedAsync(string? search,Expression<Func<TEntity, bool>>? baseFilter = null,List<Expression<Func<TEntity, string>>>? searchProperties = null,Func<IQueryable<TEntity>, IOrderedQueryable<TEntity>>? orderBy = null,int? skip = null,int? take = null,CancellationToken cancellationToken = default,params Expression<Func<TEntity, object>>[] includes)
        {
            IQueryable<TEntity> query = _dbSet.AsQueryable();
            ...
            // 4️⃣ Apply ordering to utilize indexes
            if (orderBy != null)
                query = orderBy(query);
        ...
        public async Task<List<TEntity>> GetTrashedAsync(Expression<Func<TEntity, bool>> filter,CancellationToken cancellationToken = default, params Expression<Func<TEntity, object>>[] includes)
        {
            IQueryable<TEntity> query = _dbSet
                .Where(e => EF.Property<bool>(e, "IsDeleted"))
                .Where(filter);

            // Apply includes if any are specified
            if (includes != null && includes.Any())
            {
                foreach (var include in includes)
                {
        ...
        public void UpdateCollection<TNavEntity>(ICollection<TNavEntity> existingCollection, IEnumerable<Guid> newIds, Func<Guid, TNavEntity> createEntityFunc) where TNavEntity : class
        {
        ...
        public async Task<List<TResult>> GetXListByYAsync<TFilter, TResult>(Expression<Func<TEntity, TFilter>> yProperty, TFilter yValue, Expression<Func<TEntity, TResult>> xProperty, CancellationToken cancellationToken = default)
        {
            ...
            var filterPropertyName = ((MemberExpression)yProperty.Body).Member.Name;
        ...
        public async Task<int> CountWithNavigationAsync<TNavigation>(Expression<Func<TEntity, bool>> predicate,Expression<Func<TEntity, TNavigation?>> navigationProperty,Expression<Func<TNavigation, bool>>? navigationPredicate = null,CancellationToken cancellationToken = default) where TNavigation : class
        {
            IQueryable<TEntity> query = _dbSet;
